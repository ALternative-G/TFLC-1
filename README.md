<img width="1925" height="1119" alt="image" src="https://github.com/user-attachments/assets/909461bc-b443-422f-86fd-71ee98860c41" /># TFLC-1: Разработка GUI для языкового процессора

---

## Описание проекта
Проект представляет собой текстовый редактор с графическим интерфейсом приспособленым для будущего языкового процессора с модулями сканера, парсера и потенциалом для интеграции других.

На текущем этапе реализован полноценный пользовательский интерфейс с возможностью работы с несколькими файлами одновременно, настраиваемым отображением и полной локализацией на русском и английском языках.

### Автор
* **ФИО:** Арифуллин Алан Дмитриевич
* **Группа:** АП-327

---

## Используемые технологии

*   **Язык:** C#
*   **Платформа:** .NET Framework
*   **GUI Фреймворк:** Windows Forms
*   **IDE:** Microsoft Visual Studio

---

## Интерфейс программы
Скриншоты работы программы можно посмотреть на вики

### Основные элементы интерфейса:
- **Главное меню** - полный набор функций для работы с файлами и текстом
- **Панель инструментов** - быстрый доступ к основным операциям
- **Система вкладок** - работа с несколькими файлами для ввода и программами для вывода одновременно
- **Панель нумерации строк** - автоматическая нумерация для каждого файла
- **Строка состояния** - отображение текущего статуса и информации


---

## Инструкция по установке и запуску
1. Перейдите в папку `bin\Release` или `bin\Debug`
2. Запустите файл `TFLC_GUI.exe` двойным щелчком мыши


### Системные требования:
- Запуск релизного файла должен быть возможен без предустановки IDE и иных компонентов

---

## Руководство пользователя

### Меню "Файл"
| Функция | Горячие клавиши | Описание |
|:---|:---:|:---|
| **Создать** | `Ctrl` + `N` | Создает новый пустой файл в текущей вкладке |
| **Новая вкладка** | `Ctrl` + `Shift` + `T` | Создает новую пустую вкладку |
| **Открыть** | `Ctrl` + `O` | Открывает диалог выбора файла(ов) |
| **Сохранить** | `Ctrl` + `S` | Сохраняет текущие изменения в открытом файле |
| **Сохранить как** | — | Сохраняет текущий текст в новый файл |
| **Закрыть вкладку** | `Ctrl` + `W` | Закрывает текущую вкладку |
| **Выход** | `Alt` + `F4` | Закрывает программу (с проверкой всех вкладок) |

### Меню "Правка" и Горячие клавиши

| Функция | Горячие клавиши | Описание |
|:---|:---:|:---|
| **Отменить** | `Ctrl` + `Z` | Отменяет последнее действие |
| **Повторить** | `Ctrl` + `Y` | Повторяет ранее отмененное действие |
| **Вырезать** | `Ctrl` + `X` | Удаляет выделенный текст и помещает его в буфер обмена |
| **Копировать** | `Ctrl` + `C` | Копирует выделенный текст в буфер обмена |
| **Вставить** | `Ctrl` + `V` | Вставляет текст из буфера обмена |
| **Удалить** | `Ctrl` + `D` | Удаляет выделенный текст без сохранения в буфер |
| **Выделить всё** | `Ctrl` + `A` | Выделяет весь текст в активной вкладке |

### Меню "Вид"
| Функция | Описание |
|:---|:---|
| **Увеличить шрифт** | Увеличивает размер шрифта (до 72pt) |
| **Уменьшить шрифт** | Уменьшает размер шрифта (до 6pt) |
| **Сбросить шрифт** | Возвращает стандартный размер шрифта (10pt) |

### Меню "Локализация"
| Функция | Описание |
|:---|:---|
| **Русский** | Переключает интерфейс на русский язык |
| **English** | Переключает интерфейс на английский язык |

### Меню "Текст" (Шаблонные разделы)
- Постановка задачи
- Грамматика
- Классификация грамматики
- Метод анализа
- Тестовый пример
- Список литературы
- Исходный код программы

### Меню "Справка"
| Функция | Описание |
|:---|:---|
| **Вызов справки** | Вызывает окно с подробным руководством пользователя |
| **О программе** | Информация о разработчике и версии ПО |

---

## Дополнительные возможности

### Drag & Drop поддержка
- Возможность перетащить файлы в окно программы для быстрого открытия
- `Ctrl` + перетаскивание - автоматическое открытие в новой вкладке
- Поддержка множественного выбора файлов

### Подсветка синтаксиса
Подсветка ключевых слов языков программирования (при нажатии опции меню "пуск"):
- Управляющие конструкции (`if`, `for`, `while`, `switch`)
- Модификаторы доступа (`public`, `private`, `protected`)
- Типы данных (`int`, `string`, `bool`, `float`, `double`)

### Нумерация строк
- Автоматическая нумерация для каждой вкладки
- Синхронизация прокрутки с основным текстом

### Работа с несколькими файлами
- Каждая вкладка сохраняет свою историю отмены/повтора
- Индикатор изменений (*) для модифицированных файлов

### Панели результатов
- **Сканер** - таблица для вывода результатов лексического анализа
- **Парсер** - таблица для вывода результатов синтаксического анализа
- **Оптимизация** - таблица для вывода результатов оптимизации

---
## Лексический сканнер
### Схема конечногго автомата:
<img width="531" height="533" alt="Сканер" src="https://github.com/user-attachments/assets/e53720bf-8fa4-4365-85f8-b8a7660ad2e9" />

### Шаблон лексики сканера:
START
CONST [возможные написания: "const" / "constexpr"]
SPACE_AFTER_CONST [" "]
INT ["int"]
SPACE_AFTER_INT [" "]
ID ["[letter][letters/digits]"]
EQUALS ["="]
NEGATIVE ["-"]
NUMBER ["[digits]"]
END_OF_LINE [";"]
ERROR []

### Инструкция по использованию:
1. Введите инициализацию константы в одну из строк по указанному выше формату.
2. Нажмите "пуск" или иконку с кодом.
3. Как только анализ будет выполнен появиться возможность посмотреть на обработанные токены и ошибки.
5. При нажатии на отвечающую за позицию токена/ошибки ячейку таблицы каретка будет перенесена на начало фрагмента кода.
<img width="2039" height="1227" alt="image" src="https://github.com/user-attachments/assets/6b23b0e8-a0e1-4318-8f68-01734732be37" />
Предложение по корректировке:
<img width="430" height="91" alt="image" src="https://github.com/user-attachments/assets/d7b65dcb-b87e-4f85-b0bc-3f25bf3b2f29" />

---
## Лабораторная работа 3. Разработка синтаксического анализатора (парсера)

### Цель работы
Изучить назначение и принципы работы синтаксического анализатора в структуре компилятора. Спроектировать грамматику, построить соответствующую схему метода анализа грамматики и выполнить программную реализацию парсера с нейтрализацией синтаксических ошибок методом Айронса. Интегрировать разработанный модуль в ранее созданный графический интерфейс языкового процессора.

### Вариант задания
Объявление целочисленной константы с инициализацией на языке C/C++
Корректные вводные строки:
"const int x=9;"
"constexpr int ab12c=-987;"

### Грамматика:
Класификация грамматики: автоматная грамматика

G[Start] = {Vt, Vn, P, Z}

Vt = {a, b, c... x, y, z, A, B, C... X, Y, Z, _, 0, 1, 2... 8, 9}
Vn = {Start, InConst, SpaceAfterConst, InInt, SpaceAfterInt, InID, Equals, Negative, InNumber, EndLine}
P = {
1) Start -> "const"InConst
2) InConst -> " "SpaceAfterConst
3) SpaceAfterConst -> "int"InInt
4) InInt -> " "SpaceAfterInt
5) SpaceAfterInt -> letterInID
6) InID -> "="Equals|letterInID|digitInID
7) Equals -> "-"Negative|digitInNumber
8) Negative -> digitInNumber
9) InNumber -> ";"EndLine


### Метод анализа - граф автоматной грамматики.
<img width="841" height="105" alt="Парсер" src="https://github.com/user-attachments/assets/525c0dfe-0f4b-457f-a343-9125d6a756bc" />

### Инструкция по использованию:
1. Введите инициализацию константы в одну из строк по указанному выше формату.
2. Нажмите "пуск" или иконку с кодом.
3. В строке статуса (левый верхний угол) будет отображено количество обнаруженных ошибок.
<img width="1925" height="1119" alt="image" src="https://github.com/user-attachments/assets/05433c1e-ec29-45c7-bbe7-13f3b7c3fe4e" />
4. Если ошибки есть, переключитесь на вкладку "парсер" чтобы их увидеть.
5. При нажатии на отвечающую за позицию ошибки ячейку таблицы каретка будет перенесена на начало фрагмента кода.

### Диагностика и нейтрализация ошибок
Осуществляется по показанному в алгоритме Айронса принципу.
Программа способна пропускать, дополнять и заменять проблемные участки кода и сообщать о рекомендациях пользователю.

### Тестовые примеры gримеры:
<img width="1932" height="1121" alt="image" src="https://github.com/user-attachments/assets/a43bb746-8bc1-43eb-b07a-48c495b8701a" />

<img width="1924" height="1114" alt="image" src="https://github.com/user-attachments/assets/9a20cb7d-bf37-4082-a753-73902cacf6a4" />

<img width="1929" height="1114" alt="image" src="https://github.com/user-attachments/assets/1a6db241-5261-486c-8586-c8f34dca0773" />

<img width="1925" height="1114" alt="image" src="https://github.com/user-attachments/assets/251b0d45-09b6-4d37-a3a5-c9a6f10c1361" />


<img width="1926" height="1120" alt="image" src="https://github.com/user-attachments/assets/891a199b-ac00-4926-8595-cef75b10e2e4" />

<img width="1927" height="1119" alt="image" src="https://github.com/user-attachments/assets/07b32f4d-478a-413d-b6a7-b137d9e9f682" />

---

## Лабораторная работа 4. Реализация алгоритма поиска подстрок с помощью регулярных выражений

### Цель работы
Изучить теоретические основы регулярных выражений и их применение для поиска и извлечения подстрок из текста. Освоить практические навыки использования библиотечных средств работы с регулярными выражениями, а также интеграцию алгоритмов поиска в графический интерфейс приложения.

### Вариант задания
1. Построить РВ для того, чтобы сопоставить все числа, которые не заканчиваются на 5.
2. Построить РВ, описывающее комментарии (язык C++).
3. Построить РВ, описывающее цветовой код RGB (цветовой код действителен, когда все числа находятся в диапазоне от 0 до 255]). Формат: RGB(255 120 0)

### Решение задач:
1. `\d*[123467890]`
`\d` - число
`*` - в любом количестве
[123467890] - одно из чисел не являющихся 5

2. `(//.*)|([/][*](.*(ПЕРЕВОД_СТРОКИ)*.*)*[*][/])`
`(` `)`- объединение в группу
`/` - терминальный символ `/`
`.` - любой символ
`|` - либо
`[*]` - терминальный символ `*`
ПЕРЕВОД_СТРОКИ - набор терминальных символов обозначающий переход на следующую строку. В частности - `\n`
(.*(ПЕРЕВОД_СТРОКИ)*.*)* - призвольное количество символов перед произвольным количеством переводов строк повторённое произвольное количество раз

3. RGB\((([1-2]5[0-5])|([1-2][0-4]\d)|([1-9]\d)|(\d))(\s(([1-2]5[0-5])|([1-2][0-4]\d)|([1-9]\d)|(\d))){2}\)
RGB - терминальные символы
`\` - экранирование следующего символа
`[n-m]` - любое число от n до m включительно
`\s` - пробел
`{n}` - повторить n раз

### Примеры строк.
1. Корректные:
`"123"`
`"255"` (найдётся только `"2"`)
   Некорректные:
`"5"`
`"aaa"`

2. Корректные:
`"//aaa"`
`"/* b \n c \n\n*/"`
   Некорректные:
`"/d"`
`"/* e/*"`

3. Корректные:
`"RGB(255 123 0)"`
   Некорректные:
`"RGB(256 123 0)"`
`"RGB(250 05 0)"`

### Тестовые примеры:
<img width="2017" height="1142" alt="image" src="https://github.com/user-attachments/assets/a9693cc0-5fa0-4a4c-a4b3-19a3e3e25c1c" />

<img width="2018" height="1147" alt="image" src="https://github.com/user-attachments/assets/1b20aa85-05e4-4e9c-a873-e18a5ebbf693" />


### Граф автомата для заданий 2 и 3 соответственно

<img width="701" height="487" alt="Диаграмма без названия" src="https://github.com/user-attachments/assets/28c55f15-d0d3-4026-baca-9dd53905a289" />


---

## Текущий статус разработки

### Реализовано:
- Основное меню программы.
- Панель инструментов с кнопками для быстрого доступа к часто используемым функциям.
- Область ввода/редактирования текста (текстовый редактор).
- Область отображения результатов работы языкового процессора (только для вывода, без возможности редактирования).
- Пользователь имеет возможность изменять размеры оконных областей и всего приложения. При необходимости автоматически появляются полосы прокрутки.
- Меню "Файл": Создание, открытие, сохранение, сохранение как, выход (с подтверждением сохранения изменений).
- Меню "Правка": Стандартные операции с текстом: отмена/возврат, вырезание, копирование, вставка, удаление, выделение всего текста.
- Меню "Справка": Вызов справочной системы (на данном этапе – описание реализованных функций) и окна "О программе".
- Все функции из меню "Файл", "Правка" и "Справка" доступны через соответствующие кнопки на панели инструментов.
- Приложение самодостато и должно запускаться на целевой машине без установленной среды разработки (IDE) и библиотек.
- Интерфейс с вкладками, позволяющий одновременно работать с несколькими текстами (для окна редактирования).
- Выбор языка интерфейса приложения (интернационализация).
- Нумерация строк в окне редактирования текста.
- Открытие файла при перетаскивании иконки в окно программы.
- Наличие строки состояния для отображения текущей информации о состоянии работы приложения.
- Базовая подсветка синтаксиса в окне редактирования.
- Интерфейс с вкладками, позволяющий работать с разными модулями программы (для окна вывода результатов)
- Отображение ошибок в окне вывода результатов в виде таблицы.
- Горячие клавиши для быстрых команд.
- Лексический сканнер.
- Синтаксический анализатор.
- Распознание регулярных выражений.

### В разработке:
- Функциональные компоненты  

